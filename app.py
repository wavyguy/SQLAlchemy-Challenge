...
def precipitation():
    ...
    prcp_twelve_months = Session.query(Measurement.date, Measurement.prcp).\
...
def given_start(start,end=None):
    if end==None:
        start_func = Session.query(func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)).\
